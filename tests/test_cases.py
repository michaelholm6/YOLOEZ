...
def test_labeling_workflow_runs_without_gui(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("labelling_workflow.main_labelling.crop_and_mask_images",lambda images, areas: {p: None for p in images})
    monkeypatch.setattr("labelling_workflow.main_labelling.run_contour_editor",lambda images, *a, **k: {k: [] for k in images})
    monkeypatch.setattr("labelling_workflow.main_labelling.run_box_editor", lambda *args, **kwargs: [{}]*len(test_inputs["image_paths"]))
    monkeypatch.setattr("labelling_workflow.main_labelling.run_yolo_on_crops",lambda images, *args, **kwargs: {k: [] for k in images})
...
def test_training_workflow_runs_without_gui(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("training_workflow.apply_augmentations.augment_yolo_dataset", lambda *args, **kwargs: None)
    monkeypatch.setattr("training_workflow.create_yaml.create_yaml", lambda output_dir: str(tmp_path / "dataset.yaml"))
    monkeypatch.setattr("training_workflow.main_training.run_training", lambda *args, **kwargs: {"mock_result": True})
    monkeypatch.setattr("training_workflow.dataset_chooser.split_dataset", lambda path, **kwargs: path)
...
def test_inference_workflow_runs_without_gui(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("inference_workflow.main_inference.show_instructions", lambda *a, **k: None)
    monkeypatch.setattr(
    ...
    monkeypatch.setattr("inference_workflow.main_inference.run_yolo_inference", lambda *a, **k: None)
    monkeypatch.setattr("os.makedirs", lambda path, exist_ok=True: None)  # prevent folder creation
