...
class PreprocessGUI(QtWidgets.QWidget):
    ...
    def init_ui(self):
        ...
        self.blur_spin.setSingleStep(1)
        ...
        self.clip_slider.setRange(0, 1000)
        ...
        self.clip_spin.setRange(0, 10.0)
        ...
        self.tile_slider.setRange(0, 20)
    ...
    def update_from_sliders(self):
        blur_value = self.blur_slider.value()  # keep user’s chosen value
        clip_value = self.clip_slider.value() / 100.0
        tile_value = self.tile_slider.value()

        # Update spin boxes without changing slider value
        ...
        # Internally store the value to use in cv2.GaussianBlur
        self.blur_kernel_size = blur_value if blur_value % 2 == 1 else blur_value + 1
    ...
    def update_from_spins(self):
        ...
        # Update sliders without changing spin value
    ...
    def update_image(self):
        ...
        if self.clip_limit > 0 and self.tile_grid_size > 0:
...
def run_preprocess_gui(image_path):
    ...
    return result.get('original'), result.get('post_processed')